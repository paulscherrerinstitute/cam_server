...
class PpolPeakValley():
    ...
    def ppol_interpol(self, plot_flag=False):
        ipv = [0.0, 0.0164, 0.0328, 0.0492, 0.0656,
               0.082, 0.157, 0.296, 0.42, 0.53, 0.706, 0.833, 0.918, 0.97, 0.995]
        sig = [0.0,  1.254192, 2.508384, 3.7626, 5.016768,
               6.27096483804526, 8.86848352312841, 12.5419296760905,
               15.3606640481458, 17.7369670462568, 21.723259423944,
               25.083859352181, 28.0446073247603, 30.7213280962917,
               33.1828268837964]
...
def calculate_emittance(image, fit_pars):
    ...
    _logger.info("max indices /peaks " + str(max_indices)  + " " + str(peaks))

    if len(peaks) != 2:
            mess = "Too few peaks found! " if len(peaks) < 2 else \
                   "Too many peaks found "
            _logger.info(mess + str(peaks))
            peaks_buffer = []
            for val in peaks:
    ...
    if (peaks[1] - peaks[0]) < 6:
        _logger.info("Peaks are too close: " + str(peaks[1] - peaks[0]))
    ...
    _logger.info("ratio=%f emittance %f ratio2=%f emittance2 %f" % (ratio_corrected, emittance, ratio_max, emittance2))
...
def process_image(image, pulse_id, timestamp, x_axis, y_axis, parameters, bsdata=None):
    ...
    try:
        ...
        if ret is not None:
         ...
         status = "Error in " + fname + " line " + str(exc_tb.tb_lineno) + ": " + str(e)
    ret ={}
